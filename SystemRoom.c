...
//###see error in the return values!!!
SystemResult systemRoomAdd(SystemRoom system, char *building_name, int number, int size, bool mm){
    assert(system && building_name);
    int len = strlen(building_name) + 1;
    char *string_name = malloc(sizeof(char)*len);
    if (string_name == NULL) {
        return SYSTEM_MEMORY_ERROR;
    }

    Room new_room = roomCreate(string_name, number, size, mm);
    // .....................................
}
//------------------------------------------------------------------------------------------
...
SystemResult systemDispaly(SystemRoom system, char *out_file);
